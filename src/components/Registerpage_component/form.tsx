  ...
  return(
     <div>
                  <form className=" flex flex-col  mt-3" onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <input {...register("fullName",{required:true})} id="Full Name" className=" mt-1 w-full p-2 rounded-lg border border-gray-300 focus:outline-none bg-white focus:ring-2 focus:ring-blue-200 " type="text"  />
                    {errors.fullName && <span className=" text-red-500">This field is required</span>}
                      </div>
                   <div>
                     <label htmlFor="Phone Number" className="ml-1" >Phone Number</label>
                    <input {...register("phoneNumber",{required:true, minLength:10})} id="Phone Number" className=" mt-1 w-full p-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 bg-white focus:ring-blue-200" type="text" />
                    {errors.phoneNumber && <span className=" text-red-500">This field is required and should be at least 10 digits</span>}
                   </div>
                    ...
                    <input  {...register("confirmPassword",{required:true,minLength:8,
                      ...
                      })}  id=" Confirm Password" className=" mt-1 w-full p-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 bg-white focus:ring-blue-200" type={show?"text":"password"}  />
                      {errors.confirmPassword && <span className=" text-red-500">{errors.confirmPassword.message as string}</span>}
                     <div onClick={handleClick}>
                          {!show ? <FaRegEyeSlash className=" absolute top-10 right-1     cursor-pointer"/> : <FaRegEye className="absolute right-1 top-10  cursor-pointer"/>}
                     </div>
                     ...
                     <p className="text-sm text-light ml-1">
                      <input {...register("agreed",{required:true})} type="checkbox" className="mr-1" />By signing up, I agree to our <a href="youtube.com" className="text-blue-500 underline px-1"  target="_blank " >Terms & Conditions </a> and <a href="" className="text-blue-500 underline px-1" target="_blank">Priacy Policy</a></p>
                     {errors.agreed && <span className=" text-red-500">You must agree before submitting.</span>}
